...
main (int argc, char ** argv)
{
  ...
  pcl::console::print_info ("Loading file %s ", argv[1]);
  pcl::io::loadPCDFile(argv[1], *cloud);
  pcl::console::print_info ("Loaded %s (%zu points)\n", argv[1], cloud->size ());
  cerr << "done" << argv[1] << endl;

  
  // Estimate surface normals
  SurfaceNormalsPtr normals;
  int normal_K;
  bool estimate_surface_normals = pcl::console::parse_argument (argc, argv, "-n", normal_K) > 0;
  
  if (estimate_surface_normals)
  {  
    cerr << "computing normals" << endl;
    normals = estimateSurfaceNormals (cloud, normal_K);
