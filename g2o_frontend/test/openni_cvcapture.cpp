...
void saveStuff(int count, const string& basename, Mat image, Mat depth, float imageFocalLength){
  
  char buf[100];
  sprintf(buf, "%s-%03d-GRAY.pgm", basename.c_str(), count);
  imwrite(buf, image);
  sprintf(buf, "%s-%03d-DEPT.pgm", basename.c_str(), count);
  imwrite(buf, depth);
  // unsigned char* iptr=reinterpret_cast<unsigned char*>(image.data);
  // unsigned short* dptr=reinterpret_cast<unsigned short*>(depth.data);
  // assert(image.rows==depth.rows && image.cols==depth.cols);
  ...
  // makePointCloud(cloud, image, depth, imageFocalLength);
  // sprintf(buf, "%s-%03d-XYZI.pcd", basename.c_str(), count);
...
int main( int argc, char* argv[] )
{
  ...
  if( capture.get( CV_CAP_OPENNI_IMAGE_GENERATOR_PRESENT ) )
	...
	"BASELINE" << capture.get( CV_CAP_OPENNI_DEPTH_GENERATOR + CV_CAP_PROP_OPENNI_BASELINE ) << endl <<
	"DEPTH_FOCAL_LENGHT" << (focalLength=capture.get( CV_CAP_OPENNI_DEPTH_GENERATOR + CV_CAP_PROP_OPENNI_FOCAL_LENGTH )) << endl;
  ...
  for(;;)
    {
      ...
      int c=waitKey( 30 );
      if( c == 's' || continuous ){
	cerr << "save "<< imcount << endl;
	/*
	char buf[100];
	sprintf(buf, "k-%03d-GRAY.pgm", imcount);
	imwrite(buf, grayImage);
	sprintf(buf, "k-%03d-DEPT.pgm", imcount);
	imwrite(buf, depthMap);
	*/
	saveStuff(imcount, "test", grayImage, depthMap, focalLength);
	imcount++;
      } if( c == 'c' ) {
