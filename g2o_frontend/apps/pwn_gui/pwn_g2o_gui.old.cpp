...
SparseOptimizer* loadGraph(const std::string& filename) {
  // graph construction
...
void populateListWidget(SparseOptimizer* graph, QListWidget* itemList, std::vector<RGBDData*>& listAssociations){
...
int main(int argc, char** argv) {
  ...
  SparseOptimizer* graph=loadGraph(g2oFilename.c_str());
  std::vector<RGBDData*> listAssociations;
  populateListWidget(graph, pwnGMW.listWidget, listAssociations);
