...
bool findCorrespondences(LineCorrsVector& _lcorrsVector, LinesForMatchingVector& _linesSets){
	LineCorrs currCorrs;
// 	double th = 1e3;
	cerr << "number of pairs to be matched: " << _linesSets.size() << endl;
...
void mergeLineVertex(OptimizableGraph* graph, OptimizableGraph* graphline, VertexLine2D* lNew, VertexLine2D* lOld){
	...
	if(count == 1 && lNew->p2Id > -1){
		...
		mergePointVertex(graph, graphline, dynamic_cast<VertexPointXY*>(graph->vertex(lNew->p2Id)), 
						    vlOldp2);
	}	
	    
	if (lOld->p1Id > -1 && lNew->p1Id > -1){ // merge the first vertices of both lines
		cerr << "merging the first point vertices..." << endl;
		mergePointVertex(graph, graphline, dynamic_cast<VertexPointXY*>(graph->vertex(lNew->p1Id)),
...
// 		ofstream osporca("Line2AppenaLetto.dat");
#endif
...
// 					cout << "- Line of the next vertex " << nextvertexLine << ": id "  << vl_next->id() << ": theta " << liv_next.line(0) << ", rho " << liv_next.line(1) << endl;
		...
		//debug
#if 0
					Vector2d line2 = Vector2d(vl_next->estimate());
					VertexPointXY* vpl1_2_ = dynamic_cast<VertexPointXY*>(graph->vertex(vl_next->p1Id));
					VertexPointXY* vpl2_2_ = dynamic_cast<VertexPointXY*>(graph->vertex(vl_next->p2Id));
					...
					osporca << p1line2.transpose() << endl;
					osporca << p2line2.transpose() << endl;
					osporca << endl;
					osporca << endl;
					osporca.flush();
#endif
...
// 	cerr << "edgeset tot " << counttot << endl;
	...
	cout << endl << "\033[22;31;1m....Starting correspondences finder..(SIZE OF THE FINAL CONTAINER: " << lineMatchingContainer.size() << ")\033[0m" << endl;
	...
	if(resultCorrespondances){
		//call aligner
		cout << "\033[22;31;1m....End of correspondances finder: " << lcorrsVector.size() << " pairs of vertices to be matched\033[0m"  << endl << endl;
		
		for (int c = 0; c <(int)lcorrsVector.size(); c++){
			cout << endl << "\033[22;31;1m**********************STARTING ALIGNMENT ALGORITHM: ITERATION " << c << "**********************\033[0m " << endl << endl;
...
// 			bool resultAligner = aligner(transform, correspondences, indices);
			...
			ransac.setInlierErrorThreshold(.15);
			ransac.setInlierStopFraction(.7);
			RansacLine2DLinear::TransformType transform = t0;
			std::vector<int> inliers;
			ScopeTime t("ransac aligned");
			cerr << "porcammerda" << endl;

			bool resultRansac = ransac(transform, inliers, true);
...
//				cerr << (_t0*res).matrix() << endl;
			...
			//debug: plotting lines frame i and lines frame j remapped with the transform found
				...
				for (int ci = 0; ci < (int)currCorrs.size(); ci++)
				{
				    VertexLine2D* vlj = dynamic_cast<VertexLine2D*>(graph->vertex(s2[currCorrs[ci].lid2].vline->id()));
				    Vector2d line2 = Vector2d(vlj->estimate());
				    VertexPointXY* vpl1_2 = dynamic_cast<VertexPointXY*>(graphline->vertex(s2[currCorrs[ci].lid2].vline->p1Id));
				    VertexPointXY* vpl2_2 = dynamic_cast<VertexPointXY*>(graphline->vertex(s2[currCorrs[ci].lid2].vline->p2Id));
				    Vector2d nline2(cos(line2(0)), sin(line2(0)));
...
// 				}
				...
				for (int ci = 0; ci < (int)/*currCorrs*/inliers.size(); ci++)
				{
				    ...
				    cout << endl << " \033[22;32;1mLines merged!\033[0m" << endl;
				    cout << endl;
...
// 					cout << "merging line: " << resultmergeLine << endl;
			...
			cout << endl << "\033[22;31;1m**********************END OF ALIGNMENT ALGORITHM: ITERATION " << c << "**********************\033[0m" << endl << endl;			
		}
	}
