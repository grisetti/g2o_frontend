...
float CharMatcherResult::matchingScore() const
{
...
void CharMatcher::convolveGrid(const CharGrid& g)
{
...
void CharMatcher::convolveScan(const Vector2fVector& ns, const Isometry2f& transform)
{
  const char* ker = _kernel.data();
  int kRows = _kernel.rows();
  int kCols = _kernel.cols();
...
void CharMatcher::integrateScan(const Vector2fVector& ns, const float& val, const Isometry2f& transform)
{
  Matrix2f matrix = transform.rotation();
  float angle = atan2(matrix(1, 0), matrix(0, 0));
  Vector2f offset = transform.translation();
...
void CharMatcher::saveConvolvedScanAsPPM(ostream& os, bool eq) const
{
  return _convolvedGrid.saveAsPPM(os, eq);
}


void CharMatcher::saveScanAsPPM(ostream& os, bool eq) const
{
  return _scanGrid.saveAsPPM(os, eq);
}


void CharMatcher::scanMatch(const Vector2fVector& s, const Vector3f& ig) {}


void CharMatcher::subsample(Vector2fVector& dest, const Vector2fVector& src)
{
