...
namespace pwn {
  ...
  public:
     ...
     *  @return camera matrix set to the pinholepointprojector. 
     *  @see setCameraMatrix()
     */
    inline const Eigen::Matrix3f& cameraMatrix() const { return _cameraMatrix; }  

    /**
     *  This method set the camera matrix to the one given in input.
     *  @param cameraMatrix_ is a constant reference to the 3x3 float matrix used to update the camera matrix. 
     *  @see cameraMatrix()
     */
    ...
    /**
     *  This method returns the inverse of camera matrix set to the pinholepointprojector.
  ...
  protected: 
     ...
     *  @see projectIntervals()
     */
    inline int _projectInterval(const int x, const int y, const float d, const float worldRadius) const {
