...
void RosTransformMessageHandler::tfMessageCallback(const tf::tfMessage::ConstPtr& msg){
  for (size_t i=0; i<msg->transforms.size(); i++){
    ...
    boss::ReferenceFrame* parentReferenceFrame = 0;
    boss::ReferenceFrame* childReferenceFrame = 0;

    std::map<std::string, boss::ReferenceFrame*>::iterator it = _context->frameMap().find(t.header.frame_id);    
    if (it == _context->frameMap().end()){
      parentReferenceFrame = new boss::ReferenceFrame(t.header.frame_id,Eigen::Isometry3d::Identity(), 0);
    ...
    if (it == _context->frameMap().end()){
      childReferenceFrame = new boss::ReferenceFrame(t.child_frame_id, Eigen::Isometry3d::Identity(), parentReferenceFrame);
